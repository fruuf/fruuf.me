...
export const applyArguments = scope => (fn, ...args) => {
...
export const createScope = () => {
  const fnMap = new Map();
  const wrapFn = (fn) => {
    if (!isFunction(fn)) { throw new Error('argument of scope must be a function'); }
    if (fnMap.has(fn)) {
      return fnMap.get(fn);
    }
...
export const collectArguments = (count, fn) => {
  ...
  const runCollectArguments = (innerCount, innerArgs) => (arg, scope) => {
    if (innerCount === 1) {
...
export const zipArguments = (keys, fn) =>
  collectArguments(keys.length, (scope, ...args) => fn(
    keys.reduce(
      (acc, cur, index) => {
        if (
          !isString(args[index]) &&
          !isBoolean(args[index]) &&
          !isNumber(args[index])
        ) {
          throw new Error(
